...
| [ ] | System › **macOS tiling**, with *Tiled windows have margins* off and the gap on | *Margins around tiled windows* is orange **Disabled** and one orange warning names the switch to turn on. With the gap off as well, the same row is green **Disabled** and the warning is gone |
| [ ] | Custom Areas, break the JSON and press **Verify** | A status row: the sentence naming what failed on the left, a red cross and **Invalid** on the right. Fix it and Verify: *N areas*, green **Valid** |
...
- [ ] On the System page and under the Gap group, the quoted macOS switches and panes read **exactly
      as System Settings names them in French**. Open System Settings beside it and compare the
