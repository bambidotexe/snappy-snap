...
| [ ] | Settings › **Health**, on a Mac where everything is as it should be | The overview row *SnappySnap* is green **Everything works**, **Check Again** under it. Then **Permissions** (*Accessibility permission*, *Notifications permission*), **macOS tiling** (its three rows), **Snapping** (*Drag detection* **Enabled**, *Drags with fn held* **Enabled**, *Drag detection pauses* **Never**, *Space changes and Mission Control* **Enabled**, *Last snap*, *Windows where a snap left them*, *Windows not put back* **0**), **Handles** (*Handles on offer*, *Smallest window sizes* with a count of apps and the built-in, measured and edited counts as its tooltip), **Custom areas**, **Compatibility** (*macOS* with its build as the tooltip, *Use hidden macOS features*, the four feature rows with their symbols as tooltips, one *Snap bar on “…”* row per display reading **Notch** on a MacBook's screen and **Island** on an external one), **App** (*Launch at login*, *Running for*, *Memory used*, *Crashes in the last 7 days* **None**, *Installed in* **Applications**), and **Report**. **No version and no update anywhere on the page.** Every row green or blue; *Launch at login* off reads blue **Disabled** |
| [ ] | Snap a window, come back to Health and press **Check Again** | The overview reads **Checking** with a spinner for about half a second, then *Last snap* reads a few seconds ago (the moment as its tooltip) and *Windows where a snap left them* counts that window. Nothing on the page moves by itself while you wait: it is read on show and on Check Again only |
| [ ] | With Health open, turn *Tiled windows have margins* off in Desktop & Dock | Within 2 s the margins row is orange, the overview reads orange **1 thing to look at**, and a warning under **macOS tiling** names the switch. Turn it back on: everything is green again. There is no fix button on Health |
| [ ] | Revoke Accessibility, quit and reopen SnappySnap, open Health | *Accessibility permission* is red **Denied** behind the stop sign, the overview is red **Not working: 1 problem**, the warning names SnappySnap under *Device Control and Data Access*, and the Snapping group has **no** *Drag detection* row (it waits for the permission). Grant it again: within about 2 s the permission is green and *Drag detection* appears, green **Enabled** |
| [ ] | Press **Copy Report** and paste it into TextEdit | The first line is `SnappySnap <version>, macOS <version>`, the second the summary, then every group's title and one line per row, `[OK]`, `[INFO]`, `[WARN]` or `[FAIL]`, the label, the word and the tooltip in brackets. No window title, no path but the app's own |
...
- [ ] On the System page, on the Health page and under the Gap group, the quoted macOS switches and
